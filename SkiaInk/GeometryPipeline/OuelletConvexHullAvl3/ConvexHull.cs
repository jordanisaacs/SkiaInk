...
namespace SkiaInk.GeometryPipeline.OuelletConvexHullAvl3
{
	...
	public class ConvexHull : IReadOnlyCollection<PolygonPoint>
	{
		...
		// ************************************************************************
		/// <summary>
		/// This function is called after a SKPoint has been verified to be inside
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private EnumConvexHullPoint ProcessPointForNotDisjointQuadrant(SKPoint point)
		{
			EnumConvexHullPoint result;
		...
		private SKPoint[] _pointsArrayForDynamicCall = new SKPoint[1];
		...
		public bool IsExists(SKPoint pt)
		{
			Quadrant q = _q1;
			do
			{
		...
		/// Otherwise it retuen default(SKPoint).
		...
		public EnumConvexHullPoint Evaluate(SKPoint pt)
		{
			if (!IsInitDone)
			{
		...
		public EnumConvexHullPoint TryAddOnePoint(SKPoint pt)
		{
			if (!IsInitDone)
			{
			...
			if (pt.X >= Right)
			{
				...
				if (pt.X > Right || pt.Y < _q4.LastPoint.Y)
				{
					...
					{
						_q4.LastPoint = pt;
						_q4.RootPoint = new SKPoint(_q4.FirstPoint.X, pt.Y);
				...
				if (pt.X > Right || pt.Y > _q1.FirstPoint.Y)
				{
					...
					{
						_q1.FirstPoint = pt;
						_q1.RootPoint = new SKPoint(_q1.LastPoint.X, pt.Y);
			...
			if (pt.Y >= Top)
			{
				...
				if (pt.Y > Top || pt.X < _q2.FirstPoint.X)
				{
					...
					{
						_q2.FirstPoint = pt;
						_q2.RootPoint = new SKPoint(pt.X, _q2.LastPoint.Y);
						var avlPoint = _q2.AddOrUpdate(pt);

						// Special case for top and bottom where sometimes, we should remove points between previous limit and new one.
						for (; ; )
						{
							AvlNode<SKPoint> nodeToRemove = avlPoint.GetPreviousNode();
							if (nodeToRemove == null)
							{
								break;
							}
			...
			if (pt.X <= Left)
			{
				...
				if (pt.X < Left || pt.Y > _q2.LastPoint.Y)
				{
					...
					{
						_q2.LastPoint = pt;
						_q2.RootPoint = new SKPoint(_q2.FirstPoint.X, pt.Y);
						var avlPoint = _q2.AddOrUpdate(pt);
				...
				if (pt.X < Left || pt.Y < _q3.FirstPoint.Y)
				{
					...
					{
						_q3.FirstPoint = pt;
						_q3.RootPoint = new SKPoint(_q3.LastPoint.X, pt.Y);
			...
			if (pt.Y <= Bottom)
			{
				...
				if (pt.Y < Bottom || pt.X < _q3.LastPoint.X)
				{
					...
					{
						_q3.LastPoint = pt;
						_q3.RootPoint = new SKPoint(pt.X, _q3.FirstPoint.Y);
						var avlPoint = _q3.AddOrUpdate(pt);
				...
				if (pt.Y < Bottom || pt.X > _q4.FirstPoint.X)
				{
					...
					{
						_q4.FirstPoint = pt;
						_q4.RootPoint = new SKPoint(pt.X, _q4.LastPoint.Y);
						var avlPoint = _q4.AddOrUpdate(pt);
		...
		// ************************************************************************
		private void InvalidateAllQuadrantPoints(Quadrant q, SKPoint pt)
		{
		...
		private void SetQuadrantLimitsOneThread(IReadOnlyList<PolygonPoint> points)
		{
			...
			float right, topLeft, topRight, left, bottomLeft, bottomRight;
			right = topLeft = topRight = left = bottomLeft = bottomRight = ptFirst.X;

			float top, rightTop, rightBottom, bottom, leftTop, leftBottom;
			...
			foreach (SKPoint pt in points)
			{
				...
				this._q1.FirstPoint = new SKPoint(right, rightTop);
				this._q1.LastPoint = new SKPoint(topRight, top);
				this._q1.RootPoint = new SKPoint(topRight, rightTop);

				this._q2.FirstPoint = new SKPoint(topLeft, top);
				this._q2.LastPoint = new SKPoint(left, leftTop);
				this._q2.RootPoint = new SKPoint(topLeft, leftTop);

				this._q3.FirstPoint = new SKPoint(left, leftBottom);
				this._q3.LastPoint = new SKPoint(bottomLeft, bottom);
				this._q3.RootPoint = new SKPoint(bottomLeft, leftBottom);

				this._q4.FirstPoint = new SKPoint(bottomRight, bottom);
				this._q4.LastPoint = new SKPoint(right, rightBottom);
				this._q4.RootPoint = new SKPoint(bottomRight, rightBottom);
			}
		}
		...
		private void SetQuadrantLimitsUsingAllThreads(IReadOnlyList<SKPoint> points)
		{
			SKPoint pt = points.First();
			_limit = new Limit(pt);
			...
			_q1.RootPoint = new SKPoint(_q1.LastPoint.X, _q1.FirstPoint.Y);
			_q2.RootPoint = new SKPoint(_q2.FirstPoint.X, _q2.LastPoint.Y);
			_q3.RootPoint = new SKPoint(_q3.LastPoint.X, _q3.FirstPoint.Y);
			_q4.RootPoint = new SKPoint(_q4.FirstPoint.X, _q4.LastPoint.Y);
		}

		// ******************************************************************
		private Limit FindLimits(IReadOnlyList<SKPoint> listOfPoint, int start, int offset, Limit limit)
		{
			for (int index = start; index < listOfPoint.Count; index += offset)
			{
				SKPoint pt = listOfPoint[index];
